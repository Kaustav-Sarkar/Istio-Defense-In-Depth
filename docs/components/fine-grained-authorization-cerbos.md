...
| it_admin | view_sensitive | — | DENY | — |
| hr_admin | view/list (hardware_asset) | — | DENY | — |
...
| hr_admin | view/list/assign/update | — | DENY | — |

### Cross-Role Summary (Cerbos + RLS)

| Role | Employee directory | PII / Financials | Hardware assets |
|------|-------------------|------------------|-----------------|
| employee | Public fields on all profiles | Self (+ manager for reports) | Own only |
| manager | Public fields on all profiles | Self + direct reports | Own only |
| hr_admin | Full HR fields | Full HR fields | None |
| it_admin | Public fields only | None | Full access |
