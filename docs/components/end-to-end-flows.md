...
sequenceDiagram
    ...
    AuthSvc->>AuthSvc: evaluate_request() → coarse policy: /api/profile/* allows [employee, manager, hr_admin, it_admin]
...
4. `evaluate_request()` normalizes the path (strips `/verify` prefix if present), determines the coarse route/role policy. For `/api/profile/*`, allowed roles are: `employee`, `manager`, `hr_admin`, `it_admin`.
