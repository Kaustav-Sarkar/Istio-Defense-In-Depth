...
| Operation | Rule |
|-----------|------|
| SELECT | `USING (true)` — all authenticated users can read all employee rows |
| INSERT/UPDATE/DELETE | `WITH CHECK` — only `hr_admin` can write |

The `USING (true)` for SELECT is intentional: the employees table contains directory information (names, titles, departments) visible to everyone. The sensitive data lives in separate tables with stricter policies.
