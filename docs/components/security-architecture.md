...
`true` = transaction-local. Variables vanish on commit/rollback. If a transaction reaches Postgres without setting context, `current_setting` returns `null`, all checks fail, access is denied.
...
```
SELECT: unrestricted — company directory is non-sensitive. WRITE: hr_admin only.
